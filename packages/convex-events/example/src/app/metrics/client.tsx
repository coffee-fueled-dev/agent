...
function Metrics() {
  ...
  return (
    <div>
      <h1>Metrics</h1>
      <p><a href="/">← Home</a></p>
      <p>Metric rules count events on append. Read counts with <code>getBatch</code>.</p>

      <h2>Todo events (grouped by eventType)</h2>
      <div>
        <button type="button" onClick={() => appendTodo("created")}>+ created</button>
        <button type="button" onClick={() => appendTodo("completed")}>+ completed</button>
        <button type="button" onClick={() => appendTodo("deleted")}>+ deleted</button>
      </div>
      {todoMetrics && (
        <table>
          <thead><tr><th>eventType</th><th>count</th><th>lastEventTime</th></tr></thead>
          <tbody>
            {Object.entries(todoMetrics).map(([key, val]) => (
              <tr key={key}>
                <td>{key}</td>
                <td>{val.count}</td>
                <td>{val.lastEventTime ? new Date(val.lastEventTime).toLocaleTimeString() : "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div>
        <button type="button" onClick={() => appendCounter("incremented")}>+ increment</button>
        <button type="button" onClick={() => appendCounter("decremented")}>+ decrement</button>
      </div>
      {counterMetrics && (
        <table>
          <thead><tr><th>streamId</th><th>count</th><th>lastEventTime</th></tr></thead>
          <tbody>
