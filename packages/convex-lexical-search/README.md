# `@very-coffee/convex-lexical-search`
...
- **`searchItems`**: identity + `sourceRef` + optional hooks (`bucketId` / `bucketType`, `supersededAt`, `sourceVersion`) for future grouping or lineage—not a general history index.
- **`searchTexts`**: one small document per searchable string (`propKey` + `text`); this table holds the **fulltext index** only.
