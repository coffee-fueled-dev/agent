...
import { threadIdentityFacts } from "../internal/facts";
import { agentMemoryHistory } from "../internal/history";
import { createAgentMemoryRag } from "../internal/rag";
import {
  buildEntryMetadata,
  type AgentMemoryGoogleConfig,
} from "../internal/shared";
import type { AgentMemorySearchResult } from "./search";
import { searchOptionsValidator, searchQueryValidator } from "./search";

const threadIdentityProjector = "agentMemory:threadIdentity";

export type ThreadIdentityEpisodeArgs = AgentMemoryGoogleConfig & {
  threadId: string;
  messageId: string;
  codeId: string;
  staticHash: string;
...
export type ThreadIdentitySearchArgs = AgentMemoryGoogleConfig & {
  threadId: string;
  query: string | number[];
  filters?: unknown[];
  limit?: number;
...
export const recordThreadIdentityEpisode = action({
  ...
  handler: async (ctx, args: ThreadIdentityEpisodeArgs) => {
    ...
    const parentEntryIds = (
      await agentMemoryHistory.heads.listHeads(ctx, {
        streamType: "threadIdentity",
        streamId: args.threadId,
      })
    ).map((head) => head.entryId);

    await agentMemoryHistory.append.append(ctx, {
      streamType: "threadIdentity",
      streamId: args.threadId,
    ...
    const totalTurns = previousTurn?.entity === entity
    ...
    const rag = createAgentMemoryRag({
...
export const searchThreadIdentityCurrent = action({
  ...
  handler: async (
    ...
    return await ctx.runAction(api.public.search.search, {
      ...searchArgs,
...
export const getThreadIdentityCurrent = query({
  ...
  handler: async (ctx, args): Promise<ThreadIdentityCurrentView> => {
    ...
    const heads = await agentMemoryHistory.heads.listHeads(ctx, {
      streamType: "threadIdentity",
      streamId: args.threadId,
    });
    return {
...
export const listThreadIdentityEvolution = query({
  ...
  handler: async (ctx, args): Promise<ThreadIdentityEvolutionView> => {
    const turns = await threadIdentityFacts.eval.orderedFacts(ctx, {
      namespace: threadIdentityFactsNamespace(args.threadId),
      entityType: "turn",
    });
    const historyPage = await agentMemoryHistory.read.listEntries(ctx, {
      streamType: "threadIdentity",
      streamId: args.threadId,
      paginationOpts: {
        cursor: null,
